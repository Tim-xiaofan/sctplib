...
typedef struct SCTPINSTANCE
{
	...
	struct SCTPINSTANCE *anotherInstance;
	void *recv_ring;
	void *send_ring;
   /*@}*/
}
...
int mdi_updateInstanceByAddress(SCTP_instance *instance,
			unsigned short newNoOfAddress, 
			union sockunion *newAddressList);
...
/*get the sctpInstanceName by transport addrs*/
unsigned int mdi_retrieveInstanceNameByTansportAddress(unsigned short localPort, union sockunion * localAddress)
{
	SCTP_instance *instance = retrieveInstanceByTansportAddress(localPort, 1, localAddress);
	if(instance == NULL)return 0;
	else return instance->sctpInstanceName;
}
/**
...
/**
 *  compareAssociationSctpInstacneName compares two associations and returns 0 if they are equal.
 ...
 */
gint compareAssociationSctpInstacneName(gconstpointer a, gconstpointer b)
{
	...
	if(((Association*)a)->sctpInstance == NULL || ((Association*)b)->sctpInstance == NULL)
	{
		event_log(VVERBOSE, "equalAssociations: found NO equal assocs with null sctpInstance!");
		return -1;
	}
	else if (((Association*)a)->sctpInstance->sctpInstanceName == 
				((Association*)b)->sctpInstance->sctpInstanceName )
	{
...
/**
 *   retrieveAssociation retrieves a association from the list using the sctpInstanceName as key.
 *   Returns NULL also if the association is marked "deleted" !
 ...
 *   @return pointer to the retrieved association, or NULL
 */
Association *retrieveAssociationBySctpInstacneName(unsigned short sctpInstanceName)
{
	if(sctpInstanceName <= 0)
	{
        event_logi(INTERNAL_EVENT_0, 
					"retrieveAssociationBySctpInstacneName:invalid sctpInstanceName",
					sctpInstanceName);
		return NULL;
	}
    Association *assocr;
    Association *assocp;
    GList* result = NULL;
	tmpAssoc.sctpInstance = (SCTP_instance *)malloc(sizeof(SCTP_instance));
	if(tmpAssoc.sctpInstance == NULL)
	{
		error_log_sys(ERROR_FATAL, (short)errno);
		return NULL;
	}
	tmpAssoc.sctpInstance->sctpInstanceName = sctpInstanceName;
    assocp = &tmpAssoc;
    event_logi(INTERNAL_EVENT_0, "retrieving association by sctpInstanceName %u", sctpInstanceName);

    result = g_list_find_custom(AssociationList, assocp, compareAssociationSctpInstacneName);
    ...
    if (result != NULL){
        ...
        event_log(INTERNAL_EVENT_0, "association indexed by sctpInstanceName not in list");
    }
    return NULL;
}
...
{/*start mdi*/
	...
	event_logiiii(VVERBOSE, "mdi_receiveMessageAtRing:got a sctp packet from ring[%s],len[%d],start[%p],obj[%p]",
				recv_ring->name, bufferLength, buffer, adl_get_dpdk_obj());
    ...
    if (currentAssociation == NULL) {
         ...
         if (rbu_datagramContains(CHUNK_SHUTDOWN_ACK, chunkArray) == TRUE) {
            ...
            event_log(VERBOSE, "mdi_receiveMsgAtRing: sending CHUNK_SHUTDOWN_COMPLETE  ");
            mdi_resetLastInfo();
			return;
        }
        ...
        if ((initPtr = rbu_findChunk(message->sctp_pdu, len, CHUNK_INIT)) != NULL) {
            if (sctpInstance != NULL) {
				...
				{
					event_log(INTERNAL_EVENT_0,
                         "mdi_receiveMsgAtRing: got INIT Message, and cannot create instances-> IGNORE");
					sendAbort = TRUE;
                	initChunk = ((SCTP_init_fixed *) & ((SCTP_init *) message->sctp_pdu)->init_fixed);
                	lastInitiateTag = ntohl(initChunk->init_tag);
                	event_logi(VERBOSE, "setting lastInitiateTag to %x ", lastInitiateTag);
...
unsigned int mdi_associatex(unsigned int SCTP_InstanceName, short initCID, void *ulp_data)
{
    ...
    unsigned short noOfaddressesInINIT;
	...
	noOfaddressesInINIT = ch_IPaddresses(initCID, supportedTypes, addressesInINIT, 
	...
	SCTP_instance *instance = retrieveInstance(SCTP_InstanceName);
	SCTP_instance *instance1 = NULL;
	if(instance == NULL)
	{
	...
	else
	{
		...
		event_logii(INTERNAL_EVENT_0, "mdi_associatex: set instances %u, %u", 
					instance->sctpInstanceName, instance1->sctpInstanceName);
	}
	unsigned short  destinationPort =  mdi_readLastDestPort();
    if (destinationPort == 0) {
            error_log(ERROR_MAJOR, "mdi_associate: destination port is zero....this is not allowed");
            LEAVE_LIBRARY("mdi_associate");
            return 0;
    }
    for (count = 0; count < instance1->noOfLocalAddresses; count++)
	{
        if(adl_filterInetAddress(&(instance1->localAddressList[count]),
						filterFlags) == FALSE)
		{
	...
	for (count = 0; count < noOfaddressesInINIT; count++)	
	{
        if(adl_filterInetAddress(addressesInINIT, filterFlags) == FALSE)
		{
	...
	if(noOfaddressesInINIT > instance->noOfLocalAddresses)
		if(mdi_updateInstanceByAddress(instance, noOfaddressesInINIT,
					addressesInINIT) == 0)
		{
			event_log(INTERNAL_EVENT_0, "mdi_associate: after update!");
			mdi_displayInstanceList(InstanceList);
		}
			...
			union sockunion *newAddressList)
{
	...
	{
		event_log(INTERNAL_EVENT_0, 
					"mdi_updateInstanceByAddress: attempt to update a null instance");
		return -1;
	}
	else if(newNoOfAddress == 0 || newAddressList == NULL)
	{
		event_log(INTERNAL_EVENT_0,
					"mdi_updateInstanceByAddress: invalid addrs");
		return -1;
	}
	else
	{
		...
		instance->localAddressList = 
			(union sockunion *)malloc(newNoOfAddress * sizeof(union sockunion));
		if(instance->localAddressList == NULL)
		{
			error_log_sys(ERROR_FATAL, (short)errno);
			return -1;
		...
		{
			memcpy(instance->localAddressList, newAddressList,
						sizeof(union sockunion) * newNoOfAddress);
			instance->noOfLocalAddresses = newNoOfAddress;
			return 0;
		}
	}
}
			...
			void *recv_ring, void *send_ring)
{
	...
	if(send_ring == NULL || recv_ring == NULL)
	{
		error_log(ERROR_MAJOR,"mdi_createInstanceByTansportAddress:invalid dpdk rings parameter");
		return SCTP_PARAMETER_PROBLEM;
	}
	with_ipv4 = TRUE;
...
void *mdi_getAnotherRecvRing(void* recv_ring)
{
	if(recv_ring == NULL)
	{
		event_log(INTERNAL_EVENT_0, "mdi_getAnotherRing: recv_ring is null");
		return NULL;
	}
...
void *mdi_getSendRing(void* recv_ring)
{
	if(recv_ring == NULL)
	{
		event_log(INTERNAL_EVENT_0, "mdi_getSendRing: recv_ring is null");
		return NULL;
	}
	struct rte_ring *tmp = (struct rte_ring*)recv_ring;
	if(strcmp(tmp->name, sctp_rring->name) == 0)
	...
	{
		event_logi(INTERNAL_EVENT_0, "mdi_getSendRing: unknow recv_ring'%s'",
					tmp->name);
		return NULL;
	}
	return NULL;
...
void mdi_writeDestinationAddresses(union sockunion addresses[MAX_NUM_ADDRESSES], int noOfAddresses)
{
	...
	else
	{
		...
		SCTP_instance *instance = currentAssociation->sctpInstance;
		if(instance == NULL)
		{
