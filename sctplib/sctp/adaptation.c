...
#define ENTER_TIMER_DISPATCHER              \
	printf("Entering timer dispatcher.\n"); \
	fflush(stdout);
#define LEAVE_TIMER_DISPATCHER              \
	printf("Leaving  timer dispatcher.\n"); \
	fflush(stdout);
#define ENTER_EVENT_DISPATCHER              \
	printf("Entering event dispatcher.\n"); \
	fflush(stdout);
...
/* will be added back later....
   static int icmp_sfd = -1;  */
/* socket fd for ICMP messages */
...
int adl_str2sockunion(guchar *str, union sockunion *su)
{
...
#endif
	if (ret > 0)
	{ /* Valid IPv4 address format. 使用ipv4协议*/
	...
	if (ret > 0)
	{ /* Valid IPv6 address format. 使用ipv6协议*/
...
int adl_sockunion2str(union sockunion *su, guchar *buf, size_t len)
{
	...
	{
		if (inet_ntop(AF_INET6, &su->sin6.sin6_addr, (char *)buf, len) == NULL)
			return 0;
...
int adl_register_fd_cb(int sfd, int eventcb_type, int event_mask,
					   void (*action)(void *, void *), void *userData)
{
	...
	if (sfd != 0)
	{
		...
		ret = WSAEventSelect(sfd, hEvent, FD_READ | FD_WRITE | FD_ACCEPT | FD_CLOSE | FD_CONNECT);
		if (ret == SOCKET_ERROR)
		{
...
int adl_receive_message(int sfd, void *dest, int maxlen, union sockunion *from, union sockunion *to)
{
	...
	if (len < 0)
		error_log(ERROR_MAJOR, "recvmsg()  failed in adl_receive_message() !");
